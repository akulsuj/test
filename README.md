...
# --- Helper Function to Force Forward Slashes ---
...
class Test_FileOperations(unittest.TestCase):
    ...
    def _build_path(self, *args):
        ...
        # Ensure generated paths also use forward slashes for comparison
        return force_forward_slashes(os.path.join(*args))

    # MODIFIED assertPathCalledWith to force forward slashes
    def assertPathCalledWith(self, mock_func, expected_args_tuple):
        """Asserts mock_func was called once with normalized path args (forced forward slashes)."""
        try:
    ...
    # MODIFIED assertPathHasCalls to force forward slashes
    ...
    def test_getinpfilenames_toprocess(self):
        ...
        full_dir_path = self._build_path(folderpath, inpLoadFolder) # Uses modified _build_path
        ...
        self.mock_listdir.assert_called_once_with(full_dir_path) # Use original path for listdir call check
        # Compare results after normalizing them as well
        self.assertEqual(sorted([force_forward_slashes(p) for p in result]),
                         sorted([force_forward_slashes(p) for p in expected_files]))
        self.assertNotIn(self._build_path(full_dir_path, 'file4.txt'), result) # _build_path now forces '/'
    ...
    def test_Downloadfilenames_toprocess_annual_schd_success(self):
        ...
        expected_dest_path = self._build_path(local_path, filename) # Uses modified _build_path
        expected_src_path = self._build_path(server_path, filename) # Uses modified _build_path
        # Compare result path after forcing slash
        self.assertEqual([force_forward_slashes(p) for p in result], [force_forward_slashes(expected_dest_path)])
        # Use modified assert helper
    ...
    def test_Downloadfilenames_toprocess_ftcgrossup_success(self):
        ...
        result = fo.Downloadfilenames_toprocess(server_path, local_path, action, year)
        expected_dest_files = [self._build_path(local_path, f) for f in files] # Uses modified _build_path
        expected_src_files = [self._build_path(server_path, f) for f in files] # Uses modified _build_path
        # Compare result paths after forcing slashes
        self.assertEqual(sorted([force_forward_slashes(p) for p in result]),
                         sorted([force_forward_slashes(p) for p in expected_dest_files]))

        # Manual verification forcing forward slashes
    ...
    def test_ExtractFilesToLoad(self, mock_getinp):
        ...
        # Use os.path.join directly here as it's not path comparison assertion
    ...
    def test_DownloadServerFilesToLoad(self, mock_download):
        ...
        # Use os.path.join directly here
    ...
    def test_Downloadfilenames_toprocess_ignores_temp_files(self):
        ...
        result = fo.Downloadfilenames_toprocess(server_path, local_path, action, year)
        expected_dest_path = self._build_path(local_path, good_file) # Uses modified _build_path
        expected_src_path = self._build_path(server_path, good_file) # Uses modified _build_path
        self.assertEqual([force_forward_slashes(p) for p in result], [force_forward_slashes(expected_dest_path)])
        # Use modified assert helper
        self.assertPathCalledWith(self.mock_is_open, (expected_src_path,))
    ...
    def test_Downloadfilenames_toprocess_ignores_wrong_type_or_extension_filter(self):
        ...
        wrong_ext_src_path = self._build_path(server_path, wrong_ext_file) # Uses modified _build_path
        good_file_src_path = self._build_path(server_path, good_file) # Uses modified _build_path
        good_file_dest_path = self._build_path(local_path, good_file) # Uses modified _build_path
