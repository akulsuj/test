...
class Test_FileOperations(unittest.TestCase):
    ...
    # MODIFIED assertPathHasCalls with defensive checks
    def assertPathHasCalls(self, mock_func, expected_calls_list, any_order=False):
        ...
        for i, expected_call in enumerate(expected_calls_list):
            # Defensive check for expected call structure
            if not (isinstance(expected_call, tuple) and len(expected_call) == 2):
                 raise TypeError(f"DEV ERROR: Expected call item {i} in test is not a valid (args, kwargs) tuple: {expected_call!r}")
            args, kwargs = expected_call
            if not isinstance(args, tuple):
                 raise TypeError(f"DEV ERROR: Expected call item {i} args in test is not a tuple: {args!r}")
            if not isinstance(kwargs, dict):
                 raise TypeError(f"DEV ERROR: Expected call item {i} kwargs in test is not a dict: {kwargs!r}")
        ...
        for i, actual_call in enumerate(actual_calls):
            # Defensive check for actual call structure (more likely source of unexpected format)
            if not (isinstance(actual_call, tuple) and len(actual_call) == 2):
                 raise TypeError(f"UNEXPECTED MOCK BEHAVIOR: Actual call item {i} from {mock_name}.call_args_list "
                                 f"is not a valid (args, kwargs) tuple: {actual_call!r}")
            args, kwargs = actual_call
            if not isinstance(args, tuple):
                 raise TypeError(f"UNEXPECTED MOCK BEHAVIOR: Actual call item {i} args from {mock_name}.call_args_list "
                                 f"is not a tuple: {args!r}")
            if not isinstance(kwargs, dict):
                 raise TypeError(f"UNEXPECTED MOCK BEHAVIOR: Actual call item {i} kwargs from {mock_name}.call_args_list "
                                 f"is not a dict: {kwargs!r}")
        ...
        if any_order:
            ...
            if actual_set != expected_set:
                 # Make sure error message uses the normalized lists for clarity
    ...
    def test_Downloadfilenames_toprocess_ftcgrossup_success(self):
        ...
        # Using the modified assertPathHasCalls helper
    ...
    # MODIFIED test_Downloadfilenames_toprocess_empty_after_initial_filter setup
    def test_Downloadfilenames_toprocess_empty_after_initial_filter(self):
        server_path, local_path, action, year = 's_path', 'l_path', 'Annual Stmt - Sch D', '2023'
        # Simulate listdir returning ONLY files that fail initial name/pattern/temp checks.
        # Exclude files that match name pattern but have wrong extension, as they might
        # trigger is_open depending on the implementation order.
        self.mock_listdir.return_value = [
            '~$tempfile.csv',               # Temp file
            'some_other_file.txt',        # Wrong extension (and likely name pattern)
            'WrongFilenameFormat.csv',    # Doesn't match SchD name pattern
            '2023WrongCompanySchD.csv',   # Doesn't match company in gvar settings (assuming JHUSA/JHIL)
            '2022JHUSASchD.csv'           # Wrong year
        ]
        with self.assertRaises(FileValidationException):
            fo.Downloadfilenames_toprocess(server_path, local_path, action, year)

        # Assertions for the "absolutely no matching files found" case:
        # Expect error E002 (no files), no file operations, one log call.
        self.mock_dbops_obj.BuildErrorMessage.assert_called_once_with('E002,,')
        self.mock_copyfile.assert_not_called() # No copy should happen
        self.mock_is_open.assert_not_called() # No file should even get to the is_open check
        self.mock_dbops_obj.insert_actionLog.assert_called_once() # Error should be logged
